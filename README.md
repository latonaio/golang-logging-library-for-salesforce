# golang-logging-library-for-salesforce

golang-logging-library-for-salesforce は Salesforce APIのリソースにおいて、 Go ランタイム の マイクロサービス の ログ を出力する際に、ログの json フォーマットを統一するためのライブラリです。
...
## golang-logging-library-for-salesforce のソースコードベース
golang-logging-library-for-salesforce は、[golang-logging-library](https://github.com/latonaio/avis-golang-logging-library) のソースコードをベースとして作成されています。  
...
```sh
go get github.com/latonaio/golang-logging-library-for-salesforce/logger@v1.0.0
```
...
require (
	github.com/latonaio/golang-logging-library-for-salesforce v1.0.0
)
```

```
go mod download   #全てインストールする場合
go mod download github.com/latonaio/golang-logging-library-for-salesforce@v1.0.0   #一部のみインストールする場合
```

#### 各マイクロサービスのソース内に以下を配置してください。

```go
import "github.com/latonaio/golang-logging-library-for-salesforce/logger"
```
